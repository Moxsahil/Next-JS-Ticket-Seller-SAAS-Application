...
export default function Ticket({ ticketId } : {ticketId: Id<"tickets">}){
    ...
    return (
        <div className={`bg-white rounded-2xl overflow-hidden shadow-xl border ${ticket.event.is_cancelled 
            ...
            <div  className="relative">
                {imageUrl && (
                    <div className="relative w-full aspect-[21/9]">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-b from-blacl/50 to-black/90" />
                    </div>
                )}
                <div className={`px-6 py-4 ${imageUrl ? "absolute bottom-0 left-0 right-0" : ticket.event.is_cancelled 
                                ? "bg-red-600" : "bg-blue-600"}`}>
                    <h2 className={`text-2xl font-bold ${imageUrl || !imageUrl ? "text-white" : "text-black"}`}>
                        {ticket.event.name}
                    </h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center text-gray-600">
                            ...
                            <div>
                                ...
                                <p className="font-medium">Rs {ticket.event.price.toFixed(2)}</p>
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-col items-center justify-center border-l border-gray-200 pl-6">
                        <div className={`bg-gray-100 p-4 rounded-xl ${ticket.event.is_cancelled ? "opacity-50" : ""}`}>
                            <QrCode 
                                value={ticket._id}
                                className="w-32 h-32"/>
                        </div>
            ...
            <div className={`${ticket.event.is_cancelled ? "bg-red-50" : "bg-gray-50"} px-6 py-4 flex justify-between items-center`}>
                <span className="text-sm text-grayy-500">
